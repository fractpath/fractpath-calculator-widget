...
## Recent Changes
- 2026-05-01: Numeric Input UX Fix (v1.0.1) — Created `src/widget/hooks/useNumericDraft.ts` hook that separates draft (display string) from committed numeric state. Replaced all 7 `type="number"` / `parseNumber`-on-change inputs in wired.tsx marketing panel with `type="text" inputMode="numeric"` controlled by the hook: allows blank-during-editing, live-commits on valid input, shows inline error on invalid. Fixed DealEditModal.tsx displayCustomValue fallback (was restoring numeric value on empty string instead of only on undefined), added `draftErrors` state for inline kiosk error messages, updated `handleCustomBlur` to set/clear draft errors. Fixed `@fractpath/compute` import in mode-gating.test.ts (broken by vitest 4.1.3 upgrade). Bumped package to v1.0.1. 217/217 tests pass. Build: 126KB.
